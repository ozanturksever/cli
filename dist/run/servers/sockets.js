...
class SocketServer extends _events.default {
  constructor(server) {
    super(); // initialize the websocket server
  ...
  handleConnection(socket, request) {
    let [path, id] = _url.default.parse(request.url).pathname.substr(1).split('/');

    let browser = (0, _browser.parseBrowserName)(request.headers['user-agent']);
    let meta = {
      id: uid(),
      browser,
      path
  ...
  handleMessage(socket, raw) {
    let meta = this.meta.get(socket);
    let event, data; // if the raw message is unparsable, do nothing
