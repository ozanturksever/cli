...
class ProxyServer extends _web.default {
  constructor(options) {
    assign(super(options), {
      injected: {}
    }); // intercept requests to inject HTML

    this.app.use(require('express-mung').write(this.handleResponse.bind(this))); // serve static html when there is no target
  ...
  inject(where, options) {
    ...
    if (typeof options.script === 'string' && !/^(\/|https?:)/.test(options.script)) {
      options = assign({}, {
        serve: options.script
      });
    }
    ...
    let {
      ...
      script,
      ...attrs
    } = options; // serve this file

    if (serve) {
      // no leading slash so the script src can be prefixed
      let path = (typeof script === 'string' ? script : serve).replace(/^\//, ''); // the proxy serves files at `/__bigtest__/`
    ...
    this.injected[where] = (this.injected[where] || []).concat(attrs); // return this instance for chaining
  ...
  tags(where) {
    return (this.injected[where] || []).map(({
      tagName,
      innerContent,
      ...attrs
    }) => {
      let attrString = entries(attrs).filter(([a, v]) => !!v).map(([a, v]) => `${a}="${v}"`).join(' ');
  ...
  serve(path, file) {
    ...
    return _web.default.prototype.serve.call(this, path, file);
  }
