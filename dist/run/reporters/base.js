...
function formatErrorStack({
  ...
  return stack.replace(RegExp([// remove any lines including the typical error message
  `(^.*(${escape(name)}).*(${escape(message)}).*$)`, // dedent the entire stack
  '(^\\s*(?!$))'].join('|'), 'gm'), '').trim();
}
/**
...
class Reporter {
  constructor({
    ...
    assign(this, {
      chalk: _chalk.default.constructor({
        enabled: colors
      }),
      options,
      out,
  ...
  write(...strings) {
    ...
    strings = strings.map(str => {
      ...
      return str.split('\n').map((line, i, lines) => {
        ...
        line = newline && !!line.trim() ? `${indent}${line}` : line; // we have a newline if this one is empty, or there are more
  ...
  error(error, indent = 2) {
    ...
    if (typeof error === 'string') {
      ...
      if (error.stack) {
        this.indent(this.indent() + indent).write('\n', c.gray(formatErrorStack(error))).indent(this.indent() - indent);
      }
    }
  ...
  process(prev, next) {
    ...
    if (!prev.started && next.started) {
      this.onStart(next.tests); // tests finished
    } else if (!prev.finished && next.finished) {
      this.onEnd(next.tests); // tests added or updated
  ...
  printTests(tests, print) {
    let indent = this._indent;
    let last; // loop over tests, printing their paths

    for (let i = 0, l = tests.length; i < l; i++) {
      let test = tests[i]; // print the nested path for this test

      for (let ii = 0, ll = test.path.length; ii < ll; ii++) {
        this.indent(indent + ii * 2); // only print a path if it defers from the last path
