...
var _binExists = _interopRequireDefault(require("./util/bin-exists"));
...
class ChildProcess {
  constructor({
    ...
    if (!(0, _descriptors.hasDescriptor)(this, 'command')) {
      properties.command = cmd;
    }

    if (!(0, _descriptors.hasDescriptor)(this, 'arguments')) {
      properties.arguments = args;
    }

    if (!(0, _descriptors.hasDescriptor)(this, 'env')) {
      properties.env = env;
    }

    if (!(0, _descriptors.hasDescriptor)(this, 'name')) {
  ...
  async run() {
    if (this.running) return; // find the first existing path or bin

    let cmd = [].concat(this.command).find(c => /\\|\//.test(c) ? (0, _fsExtra.existsSync)(c) : (0, _binExists.default)(c));
    ...
    let env = assign({}, process.env, this.env); // hangs around until the process exits and closes
    ...
    return new Promise((resolve, reject) => {
      ...
      this.process = (0, _child_process.spawn)(cmd, args, {
        env
      });
  ...
  async kill(signal) {
    ...
    try {
      await (0, _convergence.when)(() => !this.running);
    } catch (e) {
  ...
  unpipe(streams) {
    if (this.piped.has(streams)) {
      ...
      this.piped.delete(streams); // buffer last output stream with a newline
